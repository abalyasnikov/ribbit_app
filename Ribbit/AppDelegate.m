...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    ...
    // перед тем как запуститься спать 2 сек
    [NSThread sleepForTimeInterval:2];
    
    [Parse setApplicationId:@"qjzWjMLIY3pgm9ZfU1l5YBe9HN0NaQrinPOkoahp"
                  clientKey:@"blDxJ7yOarLq0UTQKGtrfBgF1pCY9qEN2Sp7T6Jl"];
...
{
    // меняем nav bar
    UIImage *navBarBackground = [UIImage imageNamed:@"navBarBackground"];
    [[UINavigationBar appearance] setBackgroundImage:navBarBackground forBarMetrics:UIBarMetricsDefault];
    
    [[UINavigationBar appearance] setTitleTextAttributes:@{NSForegroundColorAttributeName : [UIColor whiteColor]}]; // заголовки
    [[UINavigationBar appearance] setTintColor:[UIColor whiteColor]]; // кнопки
    
    
    // меняем tap bar
    // background и tintcolor меняем в IB
